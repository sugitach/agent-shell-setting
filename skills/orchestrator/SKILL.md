...
2. ユーザー指定を優先して役割とハーネスを確定する。指定がなければ planner=claude、coder=codex、reviewer=agy。親のハーネスはこの割り当てと独立。
   役割ごとの model と reasoning effort も独立して指定できる。未指定なら各ハーネスの既定値を使い、指定値を使えないハーネスへ勝手に変更・丸め込みをしない。
...
5. 選んだ方式で独立した子コンテキスト（標準サブエージェントまたは外部セッション）を起動し、役割 skill、Issue・計画、対象ディレクトリ、権限、返却先、指定された model・reasoning effort を渡す。子からの再委任は禁止。一度に動かす担当は1セッションとし、書き込み競合を避ける。
